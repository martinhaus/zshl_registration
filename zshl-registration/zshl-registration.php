 ...
 * Version:     1.1.0
 * Author:      Martin Hauskrecht
 * Author URI:  https://hauskrecht.sk
 * Text Domain: zshl
 * License:     GPL-2.0+
...
function zshl_create_initial_db_tables() {
	...
	$sql_regs = "CREATE TABLE {$wpdb->prefix}registrations (
	...
	$sql_items = "CREATE TABLE {$wpdb->prefix}teams (
		id mediumint(9) NOT NULL AUTO_INCREMENT,
		name varchar(100) NOT NULL,
    ...
    requester_ip_assigned varchar(255),
    requester_device_assigned varchar(255),
    requester_ip_removed varchar(255),
    requester_device_removed varchar(255),
...
function zshl_sign_team() {
    ...
    $lock = "LOCK TABLE {$wpdb->prefix}teams WRITE;";
    $wpdb->query($lock);
    ...
    if ($place_empty && $to_start) {
        ...
        if ($registered) {
            $wpdb->update($table_name,['active' => 0, 'date_removed' => date( "Y-m-d H:i:s" ), 'requester_ip_removed' => get_the_user_ip(), 'requester_device_removed' => get_user_agent()],
            ['reg_id' => $reg_id, 'name' => $team_name, 'active' => 1]);
        }
        $wpdb->insert($table_name,[
                ...
                'reg_id' => $reg_id,
                'user_id' => $user_id,
                'requester_ip_assigned' => get_the_user_ip(),
                'requester_device_assigned' => get_user_agent(),
        ]);
    }
    $lock = "UNLOCK TABLES;";
    $wpdb->query($lock);
 ...
 * added field for team name associated with the user
 */
